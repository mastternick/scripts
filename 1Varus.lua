...
VMenu.c:Boolean("cQ", "Always fully charge Q", true)
VMenu.c:Slider("rangeFQ","Range for Fast Q",750,1,1500,10) --added
...
function combo(unit)
	...
	if ValidTarget(unit, 1500) and not ValidTarget(unit, rangeFQ) then   --added
		...
		----------------added---------------
		if ValidTarget(unit, rangeFQ) then
	
		--Q1
		if VMenu.c.Q:Value() and Ready(_Q) and ValidTarget(unit, rangeFQ) and not qCharge and wTrack[GetObjectName(unit)] >= VMenu.c.sQ:Value() then
			CastSkillShot(_Q,GetOrigin(myHero))	
		end
